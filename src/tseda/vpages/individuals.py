...
class IndividualsPage(View):
    ...
    Attributes:
        ...
        individuals_table (param.ClassSelector): A reference to an
            `IndividualsTable` object managing individual data and filtering options.
        geomap (param.ClassSelector): A reference to a `GeoMap` object displaying
    ...
    def __panel__(self) -> pn.Column:
        ...
        def sample_sets_accordion_toggled(event):
            ...
            Otherwise, when the accordion is open, the width is set to 400 pixels.

            Arguments:
                event (param.Event): The event object triggered by the
                accordion's toggle. NOTE: event should not be provided, but Panel
