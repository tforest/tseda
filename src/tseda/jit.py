...
_DISABLE_NUMBA = os.environ.get("TSEDA_DISABLE_NUMBA", "0")

try:
    ENABLE_NUMBA = {"0": True, "1": False}[_DISABLE_NUMBA]
except KeyError as e:  # pragma: no cover
    raise KeyError(
        "Environment variable 'TSEDA_DISABLE_NUMBA' must be '0' or '1'"
    ) from e
...
if not ENABLE_NUMBA:
    logger.warning(
        "numba globally disabled for tseda; performance will be drastically"
        " reduced."
    )
