...
def _compute_mutation_inheritance_counts(
    ...
    while tree_pos.next():
        ...
        while mut_id < num_mutations and mutations_position[mut_id] < right:
            ...
            # Subtract this number of descendants from the parent
            # mutation. We are guaranteed to list parents mutations
...
def nodes(ts):
    ...
    nodes_num_mutations = np.bincount(
...
def sites(ts):
    sites_num_mutations = np.bincount(
...
def trees(ts):
    ...
    total_branch_length, num_internal_nodes, max_arity = (
        compute_per_tree_stats(ts)
    )
    ...
    site_tree_index = (
        np.searchsorted(
            ts.breakpoints(as_array=True), ts.sites_position, side="right"
        )
        - 1
    )
...
def preprocess(tszip_path, output_path, show_progress=False):
    ...
    with tqdm(
        ...
        # Preprocess the data first so we can error out before writing
        # to the file
        data = {}
        ...
        with tqdm(
            ...
            for table_name, arrays in data.items():
                for array_name, array in arrays.items():
                    ...
                    pbar.update(1)
        root.attrs["tseda"] = {"data_version": TSEDA_DATA_VERSION}
